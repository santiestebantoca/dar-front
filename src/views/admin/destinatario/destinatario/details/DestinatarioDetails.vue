...
const data_ = computed(() => [
  ...
  { label: 'Clave', value: data.value.clave_reparador || '-' },
  { label: 'Pin', value: data.value.pin || '-' },
  { label: 'Teléfono', value: data.value.telefono || '-' },
  { label: 'Correo', value: data.value.correo || '-' },
  { label: 'Activo', value: data.value.activo ? 'Sí' : 'No' },
])
const areas = computed(() => data.value.areas)
const smsgrupos = computed(() => data.value.smsgrupos)
...
<template>
  <div class="data-box">
    ...
    <div>
      <label class="data-label">Áreas asociadas</label>
      <ul class="data-ul">
    ...
    <div>
      <label class="data-label">Grupos de mensajería</label>
      <ul class="data-ul">
        <template v-if="smsgrupos.length">
          <li v-for="g in smsgrupos" :key="g.id">
