<script setup>
const props = defineProps({ zona: Object, asignaciones: Array })
...
const asignacion = useTP_Asignacion()
...
const drop = async ({ dataTransfer }) => {
  dragCounter.value = 0
  const destino = props.zona.nombre
  const params = { zona_id: props.zona.id }
  const paramsList = []
  ...
  <div class="card-custom" :class="cls" @dragenter="dragCounter++" @dragleave="dragCounter--" @dragover.prevent
    ...
    <div class="d-flex align-items-center p-2" @click="show = mobile ? !show : true">
      <div class="flex-fill text-truncate">
        <span :class="{ 'fw-semibold': show }" v-text="zona.nombre" class="text-truncate" />
      </div>
