...
import useAsignacion from '@/stores/despachador-ra-asignacion'
...
const centralesDialog = ref(null)
...
const resetSelection = () => {
  selection.value.destinatarios = selection.value.zonas = selection.value.centrales = []
}
const drop = async () => {
  const paramsList = []
  if (selection.value.centrales.length) {
    selection.value.destinatarios.forEach(destinatario => {
      selection.value.centrales.forEach(central => {
        paramsList.push({ central_id: central.id, region_id: central.region_id, destinatario_id: destinatario.id })
      })
    })
  } else if (selection.value.zonas.length) {
    selection.value.destinatarios.forEach(destinatario => {
...
<template>
  <div class="px-1">
    ...
    <Teleport v-if="target" to="#app-footer-content">
      <Transition>
        <div v-if="selection.destinatarios.length" class="footer hstack justify-content-center gap-2">
          <bs-btn-footer class="text-light" label="Central" icon="arrow-return-right" @click="centralesDialog = true" />
    ...
    <!-- Centrales Dialog -->
    <bs-dialog v-model="centralesDialog" fullSmDown @hidden="resetSelection">
      <bs-dialog-header label="Centrales" />
      <bs-dialog-body class="p-1 overflow-auto">
        <!-- Centrales List -->
        <ul class="list-group list-group-flush">
          <li v-for="i in centrales.data" :key="i.id" class="list-group-item px-2 py-c text-truncate border-0 bg-light">
            <input type="checkbox" :value="i" v-model="selection.centrales">
      ...
      <bs-dialog-footer class="footer justify-content-center">
        <bs-btn-footer class="text-light" label="Asignar" icon="check-lg" @click="drop"
          :disabled="!selection.centrales.length" />
      </bs-dialog-footer>
    </bs-dialog>
    <!-- Zonas Dialog -->
